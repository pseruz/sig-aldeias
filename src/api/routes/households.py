...
def list_households(
    ...
    if material: query = query.filter(Household.material == material)
    if has_elderly is not None: query = query.filter(Household.has_elderly == has_elderly)
    if has_children is not None: query = query.filter(Household.has_children == has_children)
    if status_filter: query = query.filter(Household.status == status_filter)
    return query.all()
